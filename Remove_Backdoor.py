...
def main(parser, transform_train, transform_test):
    ...
    for i in range(nb_iterations):
        ...
        train_loss, train_acc = NFT_Train(model=net, criterion=criterion, data_loader=clean_val_loader,
        ...
        print('{} \t {:.4f} \t {:.4f}'.format((i + 1) * args.epoch_aggregation, ASR, ACC))
...
def mask_clip(model, lower=0.01, upper=1.0):
    params = [param for name, param in model.named_parameters() if 'neuron_mask' in name]
    with torch.no_grad():
        for param in params:
            param.clamp_(lower, upper)

def NFT_Train(model, criterion, mask_opt, data_loader):
    model.train()
    total_correct = 0
    total_loss    = 0.0
    nb_samples    = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val-ratio', type=float, default=0.01, help='The fraction of the validate set')
    ...
    transform_train = transforms.Compose([
        ...
        NFTPolicy(),                       ## For Strong Augmentation
        transforms.ToTensor(),
        transforms.Normalize(MEAN_CIFAR10, STD_CIFAR10)
    ])
